...
export function useRealTimeLayout({
  ...
  useEffect(() => {
    if (realTimeProgress) {
      for (let i = 0; i < data.length; i++) {
        data[i].source.position.x = data[i].position
          ? data[i].store.position.x * realTimeProgress +
            DUMMY_DATA[i].store.position.x * (1 - realTimeProgress)
          : 0;
        data[i].source.position.y =
          data[i].store.position.y * realTimeProgress +
          DUMMY_DATA[i].store.position.y * (1 - realTimeProgress);
        data[i].source.position.z =
          data[i].store.position.z * realTimeProgress +
          DUMMY_DATA[i].store.position.z * (1 - realTimeProgress);
        data[i].source.rotation.x =
          data[i].store.rotation.x * realTimeProgress +
          DUMMY_DATA[i].store.rotation.x * (1 - realTimeProgress);
        data[i].source.rotation.y =
          data[i].store.rotation.y * realTimeProgress +
          DUMMY_DATA[i].store.rotation.y * (1 - realTimeProgress);
        data[i].source.rotation.z =
          data[i].store.rotation.z * realTimeProgress +
          DUMMY_DATA[i].store.rotation.z * (1 - realTimeProgress);

        let nextTimeProgress = Math.min(realTimeProgress + 1 / 60, 1);
        data[i].target.position.x =
          data[i].store.position.x * nextTimeProgress +
          DUMMY_DATA[i].store.position.x * (1 - nextTimeProgress);
        data[i].target.position.y =
          data[i].store.position.y * nextTimeProgress +
          DUMMY_DATA[i].store.position.y * (1 - nextTimeProgress);
        data[i].target.position.z =
          data[i].store.position.z * nextTimeProgress +
          DUMMY_DATA[i].store.position.z * (1 - nextTimeProgress);
        data[i].target.rotation.x =
          data[i].store.rotation.x * nextTimeProgress +
          DUMMY_DATA[i].store.rotation.x * (1 - nextTimeProgress);

        data[i].target.rotation.y =
          data[i].store.rotation.y * nextTimeProgress +
          DUMMY_DATA[i].store.rotation.y * (1 - nextTimeProgress);

        data[i].target.rotation.z =
          data[i].store.rotation.z * nextTimeProgress +
          DUMMY_DATA[i].store.rotation.z * (1 - nextTimeProgress);
      }
    }
  ...
  useEffect(() => {
    if (!realTimeProgress) {
      for (let i = 0; i < data.length; i++) {
        ...
        data[i].target.position.x = data[i].store.position
          ? data[i].store.position.x
          : 0;
        data[i].target.position.y = data[i].store.position
          ? data[i].store.position.y
          : 0;
        data[i].target.position.z = data[i].store.position
          ? data[i].store.position.z
          : 0;
        data[i].target.rotation.x = data[i].store.rotation
          ? data[i].store.rotation.x
          : 0;
        data[i].target.rotation.y = data[i].store.rotation
          ? data[i].store.rotation.y
          : 0;
        data[i].target.rotation.z = data[i].store.rotation
          ? data[i].store.rotation.z
          : 0;
      }
    }
  }, [data, layoutIdx, realTimeProgress]);

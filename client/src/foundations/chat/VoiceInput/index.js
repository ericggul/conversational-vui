...
import * as S1 from "@/foundations/chat/VoiceInput/interfaces/Interface1/styles";
import * as S2 from "@/foundations/chat/VoiceInput/interfaces/Interface2/styles";
...
function VoiceInput({ interfaceVersion, handleNewAudio }) {
  const { recorderState, startRecording, cancelRecording, saveRecording } = useRecorder(handleNewAudio);
    ...
    <>
      {interfaceVersion === 1 ? (
        <S1.StyledVoiceInput1>
          ...
          {initRecording && <S1.Time1>{`${formatMinutes(recordingMinutes)}:${formatSeconds(recordingSeconds)}`}</S1.Time1>}
        </S1.StyledVoiceInput1>
      ) : (
        <S2.StyledVoiceInput1>
          ...
          {initRecording ? (
            ...
            <S2.Button1 onClick={startRecording} recording={initRecording}></S2.Button1>
          )}
        </S2.StyledVoiceInput1>
      )}
    </>

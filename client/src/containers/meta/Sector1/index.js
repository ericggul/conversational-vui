import React, { forwardRef, Suspense, useMemo, useState, useRef, useLayoutEffect } from "react";
...
function Sector1() {
  const Text = () => {
    ...
    useFrame((state) => {
      if (mesh.current) {
        ...
        mesh.current.scale.setScalar(1 + Math.cos(t) * 0.1 + Math.sin(t * 0.3) * 0.2);
      }
    });
  ...
  const Intro = () => {
    return <S.IntroText>My name is Jeanyoon Choi, a student frontend developer with a year of experience, and I am excited to apply for the Meta’s FEE intern position this summer.</S.IntroText>;
  };
  ...
  return (
    <S.StyledSector1>
      <Canvas shadows dpr={[1, 2]} camera={{ fov: 100, position: [0, 0, 200] }}>
